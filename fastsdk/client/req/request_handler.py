...
from fastsdk.client.definitions.service_adress import ServiceAddress, create_service_address
...
class RequestHandler:
    ...
    async def _prepare_request_params(self, endpoint: EndPoint, *args, **kwargs) -> RequestData:
        ...
        if processed_files:
            ...
            # if a key is present in both body_params and file_params, the file_params value is used
            # this happens when a parameter is declared both, as file and as body param
            # ToDo: this is a hack. That doesn't work for mixed file and body params.
            # Possible solution: instead of sending the files as file they could be converted to base64 and
            # attached to the body_params as a string.
            file_pam_keys = []
            if isinstance(file_params, list):
                file_pam_keys = [v[0] for v in file_params if isinstance(v, tuple)]
            elif isinstance(file_params, dict):
                file_pam_keys = list(file_params.keys())
          
            request_data.body_params = {
                key: val for key, val in request_data.body_params.items()
                if key not in file_pam_keys
            }
