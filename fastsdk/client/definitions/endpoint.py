...
class EndPoint:
    ...
    @staticmethod
    def merge_update(params1: dict, params2: dict):
        """
        If a value is in both params they are merged into a Union type.
        """
        merged = copy(params1)
        for key, value in params2.items():
            if key in params1:
                merged[key] = Union[params1[key], value]
            else:
                merged[key] = value
        return merged
    ...
    def get_parameter_definition_as_dict(self):
        ...
        all_params = self.merge_update(all_params, parse(self.body_params))
        all_params = self.merge_update(all_params, parse(self.file_params))
        return all_params
