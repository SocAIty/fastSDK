...
class JobRuntime:
    ...
    def refresh_stream_state(self) -> None:
        ...
        The orchestrator calls this as job state advances. Keeping the check here
        leaves the stream-open policy in the runtime, not in the manager.
        """
    ...
    def stream(self, timeout_s: float = 60.0) -> StreamSession:
        ...
        A ``StreamSession`` serves both sync (``iter_*``) and async (``aiter_*``)
        consumers, so one entrypoint covers every caller.
        """
        with self._lock:
    ...
    def cancel(self, wait: bool = False, timeout_s: float = 30.0, poll_interval_s: float = 0.5) -> Any:
        ...
        Closes any active stream first, then drives the remote cancel. With
        ``wait`` set, polls the provider until it confirms ``CANCELLED`` or reaches
        another terminal state instead of assuming the cancel endpoint succeeded.
        """
